...
-- (housekeeping, kitchen, etc.) never need a PMS account at all, so this doesn't assume
-- every employee has one. linked_user_id is optional, for the few who do.
CREATE TABLE employees (
  id              SERIAL PRIMARY KEY,
  full_name       TEXT NOT NULL,
  position        TEXT,                         -- e.g. 'Housekeeper', 'Chef', 'Front Desk'
  phone           TEXT,
  email           TEXT,
  start_date      DATE,
  active          BOOLEAN NOT NULL DEFAULT TRUE,  -- former staff stay on record, just hidden
  notes           TEXT,
  linked_user_id  INTEGER REFERENCES users(id),
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- HR: leave requests. Recorded and actioned by a manager — there's no separate staff
-- login for most employees to submit their own, so this models "manager logs a leave
-- request on the employee's behalf and approves/rejects it," not true self-service.
...
-- HR: reusable letter templates, so a common letter (a standard offer letter, a standard
-- warning) doesn't need retyping from scratch each time — load it into the editor, adjust
-- the specifics, print.
