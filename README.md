...
- The web frontend covers login, dashboard, check-in/check-out, ID capture (Emirates ID/passport), and now prints the registration card, invoice, and gate pass directly from a success screen after each flow. Still missing from the UI: the housekeeping report, inventory, expenses, and reports/night audit — all work via the API, just no button for them yet
